require 'open3'
class Admin::ImportAccountsController < Admin::ApplicationController
  ...
  def create
    ...
    if gdrive_id.blank?
      return_value = ImportAccount.new(file).import_file(assigned, campaign)
    else
      return_value = ImportAccount.new(file).import_gdrive(gdrive_id, assigned, campaign)
    end
    error = return_value[0]
      if error == ''
        total = return_value[1]
        c = return_value[2]
        redirect_to new_admin_import_account_path, :notice => "Total Accounts for import  #{total}, #{c} duplicates skipped"
      else
        flash[:error] = t(:msg_gdrive_error, error)
        redirect_to new_admin_import_account_path
    end
  end

  def setup_current_tab
    set_current_tab('admin/import_accounts')
  end
